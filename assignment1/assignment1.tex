...
We are examining the requirements document in an attempt to detect problems with the requirements as early as possible in the development process. Large projects tend to grow out of hand in terms of time and money spent. On top of that, the later in the development process these problems are encountered, the higher the costs in terms of time and money spent are to resolve them. This is why it is useful to assess the suitability of the requirements for evolution in this early stage.
...
\paragraph{Vagueness} 
Req \ref{4}. ``frequent errors" is not a clear number.\\
Req \ref{5}. ``A pool of"  is not quantifiable.\\
...
\paragraph{Optionality}
Req \ref{45}. ``\ldots Maybe \ldots would be better to \ldots." The word ``maybe" here indicates the optionality of this part of the requirement.

\subsection{Dependencies}
Before looking at individual requirements, we looked at dependencies between features of the system. We found that all features depend on the part: ``Distance learning music education in VEMUS". This is due to the fact that the requirements stated in that part are general requirements of the system. We also found that the parts ``Teacher-student feedback", ``Providing feedback", ``studying procedures" and ``Educational content" concern for a large part the same functionality of the system, namely the ability of the system to provide automatic performance evaluation. We found that the part ``Accompanying music" is not that much affected by the other parts although it is a studying procedure. Lastly, we found that the part ``VEMUS visualizations" is largely concerned with only the view part of the system and in a model-view-controller analogy, can be changed without affecting the other parts. In that same analogy, it can be the case that this parts needs changing when some of the others change. 

Now we start investigating individual requirements. The dependencies are found by a manual form of keyword searching. We did not search the document for specific keywords but rather we read the requirements and when we encountered a keyword K in a requirement B that we had already read in requirement A, we investigated these two requirements closer in order to determine the possibility of there being a dependency relation between them.
...
\caption{Traceability graph of requirements of VEMUS \cite{VEMUS}. Nodes correspond to requirements in \ref{reqs}. Arcs correspond to dependencies. A larger image is presented in appendix \ref{graphapp}.}
\label{fig:graph}
\end{figure}
...
Another possible threat to the validity of the results is a misinterpretation of the form of some of the requirements. There are some requirements which seem to be a general statement about the context of the system but not a real requirement. This concerns requirements such as requirements \ref{20} and \ref{21}. We did not consider these requirements as system requirements.
...
\subsection{Teacher-student feedback}
	...
	\req[4]{There must be annotations on students score with remarks or to outline some sections of the score because of frequent errors or due to some complexity, while they play or after they have finished their performance.}\\
	\req[5]{Teachers must have access to a pool of different icons to be used for annotations and also to be able to design their own icons for instrument-specific purposes. Additionally the idea of audio annotations was considered of great pedagogical value.}\\
...
\subsection{Distance learning music education in VEMUS}
	\req[20]{There is a great need to access music resources in rural and remote areas}\\
	\req[21]{There is great lack of teachers of wind instruments in rural areas}\\

...
\begin{description}
	...
	\item[\texttt{console}] Opens an interactive console with the model loaded so it's possible to query the database through the \texttt{activerecord} interface. (For instance, run \texttt{Project.\-where(:processed => true)\.count} to get the number of processed projects).
